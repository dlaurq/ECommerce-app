...
export default function CartProductInteraction({
  cartProduct,
}: {
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement> | string) => {
    if (typeof e === "string") {
      if (e === "-")
        setQuantity((prevQuant) => (prevQuant - 1 <= 0 ? 1 : prevQuant - 1));
    ...
    <section className="flex flex-row justify-between items-start">
      <section>
        <section className="flex flex-row">
          <button
            className="border-gray-200 border-2 text-sm font-bold w-7 h-7 disabled:opacity-50 disabled:bg-gray-200 hover:opacity-60"
          ...
          />
          <button
            className=" border-gray-200 border-2 text-sm font-bold w-7 h-7 hover:opacity-60"
            onClick={() => handleChange("+")}
          >
